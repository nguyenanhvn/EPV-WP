		...
		<div class="content-press_box">
			<div class="box__left">
				<h2 class="heading">
					<?php 
					global $epv_options;
					$pieces = explode(" ", changeLang($epv_options['registration-title'],$epv_options['registration-title-vi']));
					for ($i = 0; $i <= count($pieces) - 1; $i++) {
						if($i == (count($pieces) - 1))
							printf(" <span>%s</span>",$pieces[$i]);
						else
				...
				<div class="paragraph">
					<?=	changeLang($epv_options['registration-text'],$epv_options['registration-text-vi']);?>
				</div>
			</div>
